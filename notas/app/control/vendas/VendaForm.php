...
class VendaForm extends TPage
{
    ...
    function __construct($param)
    {
        ...
        $this->form->addFields( [new TLabel('Dt Venda')], [$dt_venda] );
        ...
        $this->form->addContent( [ TElement::tag('h5', 'Itens da venda', [ 'style'=>'padding: 5px; margin-top: 10px'] ) ] );
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $items  = VendaItem::where('venda_id', '=', $key)->load();
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            $master->dt_venda = TDateTime::convertToMask($param['dt_venda'], 'dd/mm/yyyy', 'yyyy-mm-dd');
            $master->mes = TDateTime::convertToMask($param['dt_venda'], 'dd/mm/yyyy', 'mm');
            $master->ano = TDateTime::convertToMask($param['dt_venda'], 'dd/mm/yyyy', 'yyyy');
            
            $master->store(); // save master object
            
            // delete details
            VendaItem::where('venda_id', '=', $master->id)->delete();
            ...
            if( !empty($param['list_servico_id']) AND is_array($param['list_servico_id']) )
            {
                ...
                foreach( $param['list_servico_id'] as $row => $servico_id)
                {
                    if (!empty($servico_id))
                    {
                        $detail = new VendaItem;
                        $detail->venda_id = $master->id;
            ...
            $pos_action = new TAction(['VendaList', 'onReload']);
